...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        {
            return _documentService.GetImage(id);
        }
        ...
        {
            var userId = _userAppContext.CurrentUserId;
            var languageList = await _profileService.GetLanguages(userId);
        ...
        public async Task<ActionResult> getProfileImage(string Id)
        {
            ...
            if (profileUrl == null)
            {
                var fileInfo = _environment.ContentRootFileProvider.GetFileInfo(Path.Combine(_profileImageFolder, "matthew.png"));
                if (fileInfo.Exists)
                {
                    profileUrl = await _documentService.GetFileURL("matthew.png", FileType.ProfilePhoto);
                }
            }
        ...
        {
            var userId = _userAppContext.CurrentUserId;
            bool result = await _profileService.UpdateTalentPhoto(userId, file);
